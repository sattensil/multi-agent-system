...
You are a readability assessment specialist for translated content. Your job is to evaluate the readability of translated text with a critical eye for complexity and accessibility.

Assess the following aspects of the translation:
1. Clarity: Is the text easy to understand for the intended audience? Look for overly complex sentences, unnecessary jargon, and convoluted expressions.
2. Fluency: Does the text flow naturally in the target language? Check for awkward phrasing, unnatural word order, or stilted language.
3. Vocabulary Level: Is the vocabulary appropriate or unnecessarily complex? Identify overly academic or archaic terms that could be simplified.
4. Sentence Structure: Are sentences clear and direct, or overly long and complex with multiple clauses?
5. Consistency: Is terminology used consistently throughout the text?
6. Accessibility: Would a general audience be able to understand the main points without specialized knowledge?

Be STRICT in your assessment. Text with unnecessarily complex language, excessive jargon, or overly formal academic style should receive lower scores.

Rate the overall readability on a scale of 1-10, where:
1-3: Poor readability - Text is extremely difficult to understand due to complex language, excessive jargon, and convoluted sentence structures
4-6: Moderate readability - Text requires effort to understand and would benefit from significant simplification
7-8: Good readability - Text is generally clear with some room for improvement
9-10: Excellent readability - Text is clear, accessible, and easy to understand for the intended audience

If you encounter unnecessarily complex language like "multifarious implementations and the concomitant ramifications thereof" instead of "various uses and their effects," you should rate the text lower for poor readability.
...
# Initialize the LLMs with different models for different tasks

# Llama 3 70B for translation (best quality for complex translations)
...
# Mistral 7B for readability assessment (good for structured evaluation)
readability_llm = ChatOpenAI(
...
# Qwen 2.5 7B for revision (good balance of quality and efficiency)
reviser_llm = ChatOpenAI(
...
# Use Mistral 7B for the supervisor as well
...
def supervisor_node(state: State) -> Command[Literal["translator", "readability_tester", "reviser", END]]:
    ...
    print("\n" + "=" * 80)
    print(f"🧠 SUPERVISOR AGENT (MISTRAL-7B-INSTRUCT): Deciding next step...")
    print("-" * 80)
    ...
    if document_content and target_language and not translated_content:
        ...
        system_prompt = f"""
        You are a language detection specialist. Your task is to carefully analyze the following text and determine if it is written in {target_language}.
        
        IMPORTANT INSTRUCTIONS:
        1. Examine the vocabulary, grammar, and sentence structure of the text.
        2. Look for language-specific characteristics that would identify it as {target_language}.
        3. Do not be misled by proper nouns or technical terms that might appear in multiple languages.
        4. Consider that some languages share vocabulary but have different grammar.
        
        RESPOND WITH ONLY ONE WORD: either 'yes' or 'no'.
        - 'yes' if the text is primarily written in {target_language}
        - 'no' if the text is in any other language
        """
        ...
        print(f"🔍 SUPERVISOR AGENT (MISTRAL-7B-INSTRUCT): Checking if document is already in {target_language}...")
        response = supervisor_llm.invoke(messages)
        response_text = response.content.lower().strip()
        
        # More strict checking of the response
        is_target_language = False
        if response_text == 'yes' or response_text == 'sí' or 'yes.' in response_text:
            # Double-check with a second prompt if the model thinks it's in the target language
            verification_prompt = f"""
            You are a language verification specialist. I need a second opinion.
            
            The text below has been identified as possibly being in {target_language}.
            Please verify this and respond with ONLY 'correct' if it is indeed in {target_language},
            or 'incorrect' if it is in some other language.
            
            DO NOT include any explanation, just the single word 'correct' or 'incorrect'.
            """
            
            verification_messages = [
                SystemMessage(content=verification_prompt),
                HumanMessage(content=document_content[:500])  # Use first 500 chars for verification
            ]
            
            verification_response = supervisor_llm.invoke(verification_messages)
            verification_text = verification_response.content.lower().strip()
            
            is_target_language = 'correct' in verification_text
            
            if is_target_language:
                print(f"✅ Verified: Document is already in {target_language}, skipping translation")
            else:
                print(f"🔄 Verification failed: Document needs translation to {target_language}")
    ...
    # Since structured output might not work the same with all models,
    # we'll use a more robust approach with prompt engineering
    
    # Get current state information to help with decision making
    document_content = state.get("document_content", "")
    target_language = state.get("target_language", "")
    translated_content = state.get("translated_content", "")
    readability_score = state.get("readability_score")
    revised_content = state.get("revised_content", "")
    previous_next = state.get("next", "")
    
    # Add state tracking to prevent loops
    call_count = state.get("call_counts", {})
    
    # Logic-based decision making to supplement the LLM
    next_agent = None
    reason = ""
    
    # If we have document and language but no translation yet, go to translator
    if document_content and target_language and not translated_content:
        next_agent = "translator"
        reason = "We have document content and target language but no translation yet."
    # If we have a translation but no readability score, go to readability tester
    elif translated_content and readability_score is None:
        next_agent = "readability_tester"
        reason = "We have a translation but need to assess its readability."
    # If we have a readability score below threshold and no revision, go to reviser
    elif readability_score is not None and readability_score < 7.0 and not revised_content:
        next_agent = "reviser"
        reason = f"The readability score of {readability_score}/10 is below the threshold of 7.0."
    # If we have a readability score at or above threshold, finish
    elif readability_score is not None and readability_score >= 7.0:
        next_agent = "FINISH"
        reason = f"The translation has good readability with a score of {readability_score}/10, which meets or exceeds the threshold of 7.0."
    # If we have revised content and reassessed readability, finish
    elif revised_content and readability_score is not None:
        next_agent = "FINISH"
        reason = f"The translation has been revised and has a readability score of {readability_score}/10."
    # If we're missing critical information, finish with an error
    elif not document_content or not target_language:
        next_agent = "FINISH"
        reason = "Missing document content or target language."
    
    # If we've determined the next agent logically, use it
    if next_agent:
        goto = next_agent
        print(f"✅ LOGICAL DECISION: Call the {goto} agent")
        print(f"📝 REASON: {reason}")
        print("=" * 80)
        return Command(goto=goto if goto != "FINISH" else END, 
                      update={"next": goto, "call_counts": call_count})
    
    # If logical decision making didn't determine the next agent, use the LLM
    ...
    Where [agent name] is one of: translator, readability_tester, reviser, or FINISH
    
    CURRENT STATE:
    - Document content: {'Present' if document_content else 'Missing'}
    - Target language: {target_language if target_language else 'Not specified'}
    - Translated content: {'Present' if translated_content else 'Missing'}
    - Readability score: {readability_score if readability_score is not None else 'Not assessed'}
    - Revised content: {'Present' if revised_content else 'Missing'}
    - Previous step: {previous_next if previous_next else 'None'}
    """
    ...
    # Parse the response to extract the next agent and reason
    import re
    next_match = re.search(r"NEXT:\s*([\w]+)", response_text, re.IGNORECASE)
    reason_match = re.search(r"REASON:\s*(.+)(?:\n|$)", response_text, re.IGNORECASE | re.DOTALL)
    
    if next_match:
        goto = next_match.group(1).lower()
        # Validate the goto value
        if goto not in ["translator", "readability_tester", "reviser", "finish"]:
            goto = "translator"  # Default to translator if invalid
        
        # Convert 'finish' to 'FINISH' to match the expected format
        if goto == "finish":
            goto = "FINISH"
    else:
        # Default to readability_tester if we have translated content but no readability score
        if translated_content and readability_score is None:
            goto = "readability_tester"
        # Default to reviser if we have a low readability score
        elif readability_score is not None and readability_score < 7.0 and not revised_content:
            goto = "reviser"
        # Default to FINISH if we have good readability or revised content
        elif (readability_score is not None and readability_score >= 7.0) or revised_content:
            goto = "FINISH"
        # Default to translator if we have nothing yet
        else:
            goto = "translator"
    ...
    if reason_match:
        ...
        reason = "Moving to the next step in the workflow."
        
    # Track call counts to prevent infinite loops
    call_count = state.get("call_counts", {})
    call_count[goto] = call_count.get(goto, 0) + 1
    
    # Safety check to prevent infinite loops
    if call_count.get(goto, 0) > 3 and goto == "translator" and translated_content:
        goto = "readability_tester"
        reason = "Switching to readability testing to prevent an infinite loop."
        print(f"⚠️ Detected potential loop, forcing transition to {goto}")
    elif call_count.get(goto, 0) > 3 and goto == "readability_tester" and readability_score is not None:
        if readability_score < 7.0:
            goto = "reviser"
        else:
            goto = "FINISH"
        reason = f"Switching to {goto} to prevent an infinite loop."
        print(f"⚠️ Detected potential loop, forcing transition to {goto}")
    elif call_count.get(goto, 0) > 3:
        goto = "FINISH"
        reason = "Ending workflow to prevent an infinite loop."
        print("⚠️ Detected potential infinite loop, ending workflow")
    # We've already parsed goto and reason from the response text above
...
def translator_node(state: State) -> State:
    ...
    print("\n" + "=" * 80)
    print(f"🐤 TRANSLATOR AGENT (LLAMA-3-70B-INSTRUCT): Translating document to {target_language}...")
    print("-" * 80)
...
def readability_tester_node(state: State) -> State:
    ...
    print("\n" + "=" * 80)
    print(f"📊 READABILITY TESTER AGENT (MISTRAL-7B-INSTRUCT): Assessing {target_language} translation...")
    print("-" * 80)
    ...
    IMPORTANT: Your response MUST include a numerical score in the format 'Score: X/10' where X is a number between 1 and 10.
    Make sure to include the exact text 'Score: X/10' in your response, where X is your rating.
    """
    ...
    response = readability_llm.invoke(messages)
    feedback = response.content
    
    # Extract the readability score from the feedback using multiple patterns
    import re
    
    # Try several patterns to find a score
    patterns = [
        r"Score:\s*(\d+(?:\.\d+)?)\/10",  # Score: X/10
        r"(\d+(?:\.\d+)?)\/10",           # X/10
        r"score[^\d]+(\d+(?:\.\d+)?)",    # score X
        r"rating[^\d]+(\d+(?:\.\d+)?)",   # rating X
        r"(\d+(?:\.\d+)?)\s*out of\s*10" # X out of 10
    ]
    ...
    # If no score found, try to extract any number between 1 and 10
    if readability_score is None:
        number_matches = re.findall(r"\b(\d+(?:\.\d+)?)\b", feedback)
        for num in number_matches:
            num_val = float(num)
            if 1 <= num_val <= 10:
                readability_score = num_val
                print(f"⚠️ Using extracted number {num_val} as readability score")
                break
    
    # If still no score, use a default with a warning
    if readability_score is None:
        readability_score = 5.0  # Default middle score
        print(f"⚠️ WARNING: Could not extract a readability score from the assessment.")
        print(f"⚠️ Using default score of {readability_score}/10")
        print(f"⚠️ Raw feedback: {feedback[:200]}...")
...
def reviser_node(state: State) -> Command[Literal["readability_tester"]]:
    ...
    print("\n" + "=" * 80)
    print(f"✏️ REVISER AGENT (QWEN2.5-7B-INSTRUCT): Improving {target_language} translation...")
    print("-" * 80)
    ...
    response = reviser_llm.invoke(messages)
